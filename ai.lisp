...
(defmethod run-episode ((a agent) initial-state)
  (with-slots (stepfun actions) a
    (do* ((state initial-state next-state)
	  ...
	  (step-results (funcall stepfun action) (funcall stepfun action))
	  (next-state (aref step-results 0) (aref step-results 0))
	  (reward (aref step-results 1) (aref step-results 1))
